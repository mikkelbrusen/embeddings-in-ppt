...
class MultiStepAttention(nn.Module):
  def __init__(self, input_size, hidden_size, num_steps=10, num_layers=1):
    ...
    self.attn = Attention(in_size=input_size, hid_size=hidden_size*self.num_layers, att_size=input_size//2)
  ...
  def forward(self, x_in, hidden, seq_lengths): #x_in: (batch_size, seq_len, hidden_size*2)
    ...
    h_x = h_x.view(h_x.size(1), h_x.size(2)*self.num_layers) #(batch_size, hidden_size*2)
    c_x = c_x.view(c_x.size(1), c_x.size(2)*self.num_layers)

    for i in range(self.num_steps):
      c_vector, alpha = self.attn(x_in=x_in, h_prev=h_x, seq_lengths=seq_lengths) # c_t: (batch_size, hidden_size*2)
...
class ABLSTM(nn.Module):
  def __init__(self, batch_size, n_hid, n_feat, n_class, lr, drop_per, drop_hid, n_filt, use_cnn=False):
    ...
    self.relu = nn.ReLU()
    self.multi_attn = MultiStepAttention(input_size=n_hid*2, hidden_size=n_hid, num_steps=10, num_layers=2)
