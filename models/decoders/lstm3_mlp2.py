...
class Decoder(nn.Module):
  ...
  def __init__(self, args, in_size):
    super().__init__()
    self.args = args
    self.bi_rnn = nn.LSTM(input_size=in_size, hidden_size=self.args.n_l1, num_layers=3, bidirectional=True, batch_first=True)
    self.densel1 = nn.Linear(self.args.n_l1*2, self.args.n_l2)
