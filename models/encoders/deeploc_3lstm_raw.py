...
class Encoder(nn.Module):
  ...
  def __init__(self, args):
    ...
    self.lstm1 = nn.LSTM(128, args.n_hid, bidirectional=True, batch_first=True)
    self.lstm2 = nn.LSTM(args.n_hid*2, args.n_hid, bidirectional=True, batch_first=True)
    self.lstm3 = nn.LSTM(args.n_hid*2, args.n_hid, bidirectional=True, batch_first=True)
  ...
  def forward(self, inp, seq_lengths):    
    ...
    pack, _ = self.lstm1(pack) #h = (2, batch_size, hidden_size)
    pack, _ = self.lstm2(pack) #h = (2, batch_size, hidden_size)
    pack, _ = self.lstm3(pack) #h = (2, batch_size, hidden_size)
    output, _ = nn.utils.rnn.pad_packed_sequence(pack, batch_first=True) #(batch_size, seq_len, hidden_size*2)
